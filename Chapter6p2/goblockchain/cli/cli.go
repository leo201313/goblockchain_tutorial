...
// Now balance is realized by calling wallet.GetBalance
func (cli *CommandLine) balance(address string) {
	wlt := wallet.LoadWallet(address)
...
func (cli *CommandLine) Run() {
	...
	iniUtxoCmd := flag.NewFlagSet("initutxoset", flag.ExitOnError)
	...
	case "initutxoset":
		err := iniUtxoCmd.Parse(os.Args[2:])
		utils.Handle(err)
